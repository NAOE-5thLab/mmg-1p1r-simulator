...
principal_particulars = pd.read_csv(
    "./Hydro/MMG/input_csv/principal_particulars.csv", header=0, index_col=0
)
# read parameters from csv
parameter_init = pd.read_csv(
    "./Hydro/MMG/input_csv/MMG_params.csv", header=0, index_col=0
)
...
exp_data = pd.read_csv(
    "./Hydro/MMG/input_csv/stopping_processed_CRASH_ASTERN_1200-1200_-35.xlsx.csv",
    header=0,
    index_col=0,
)
...
stateval[0:1, 0:1] = exp_data.at[
    exp_data.index[startstep], "x_position_mid [m]"
]  # x_pos t=0
stateval[0:1, 1:2] = exp_data.at[exp_data.index[startstep], "u_velo [m/s]"]  # u t=0
stateval[0:1, 2:3] = exp_data.at[
    exp_data.index[startstep], "y_position_mid [m]"
]  # y_pos t=0
...
stateval[0:1, 5:6] = exp_data.at[
    exp_data.index[startstep], "r_angvelo [rad/s]"
]  # r t=0
...
for i in range(no_timestep - 1):
    slope = rk.rk4_mmg(
        dt_sec,
        mmg2.hydroForceLs,
        exp_data.index[startstep + i : startstep + i + 1].values,
        stateval[i : i + 1, :],
        n_prop[i : i + 1],
        delta_rudder[i : i + 1],
        windv[i : i + 1],
...
ax1.plot(
    exp_data.loc[:, "y_position_mid [m]"],
    exp_data.loc[:, "x_position_mid [m]"],
    color="black",
    linewidth=0.7,
    label="exp",
)
...
ax1.plot(
    valid_hachii_nowind[:, 2:3],
    valid_hachii_nowind[:, 0:1],
    color="tomato",
    linewidth=0.7,
    linestyle="--",
    label="Hachii's model for $X_{p},$ w/o wind",
)
for i, ti in enumerate(exp_data.index.values):
    ...
    if ti % 5 == 0:
        ...
        x1, x2, x3, x4, x5 = traj.ship_coo(
            np.array(
                [
                    exp_data.at[exp_data.index[i], "x_position_mid [m]"],
                    exp_data.at[exp_data.index[i], "y_position_mid [m]"],
                    exp_data.at[exp_data.index[i], "psi_hat [rad]"],
                ]
            ),
            lpp,
            breadth,
...
for i, ti in enumerate(exp_data.index[startstep:].values):
    ...
    if ti % 10 == 0:
        ...
        exprwindstart[0,] = exprwindstart[0] + exp_data.at[
            exp_data.index[i], "wind_velo_relative_mid [m/s]"
        ] * np.sin(
            exp_data.at[exp_data.index[i], "wind_dir_relative_mid [rad]"]
            + exp_data.at[exp_data.index[i], "psi_hat [rad]"]
        )
        exprwindstart[1,] = exprwindstart[1] + exp_data.at[
            exp_data.index[i], "wind_velo_relative_mid [m/s]"
        ] * np.cos(
            exp_data.at[exp_data.index[i], "wind_dir_relative_mid [rad]"]
            + exp_data.at[exp_data.index[i], "psi_hat [rad]"],
        ...
        hachiix1, hachiix2, hachiix3, hachiix4, hachiix5 = traj.ship_coo(
            np.array([valid_hachii[i, 0], valid_hachii[i, 2], valid_hachii[i, 4]]),
            lpp,
            breadth,
            scale=1,
        )
        hachiipoly = plt.Polygon(
            (
                hachiix1[::-1],
                hachiix2[::-1],
                hachiix3[::-1],
        ...
        (
            ...
            np.array(
                [
                    valid_hachii_nowind[i, 0],
                    valid_hachii_nowind[i, 2],
                    valid_hachii_nowind[i, 4],
                ]
            ),
            lpp,
            breadth,
...
ax1.plot(
    exp_data.index.values,
    exp_data.loc[:, "u_velo [m/s]"],
    color="black",
    linewidth=0.7,
    label="exp",
)
ax1.plot(
    exp_data.index[startstep:].values,
    valid_yasukawa[:, 1:2],
    color="#0277BD",
    linewidth=0.7,
    label="Yasukawa",
...
ax1.plot(
    exp_data.index[startstep:].values,
    valid_yasukawa_nowind[:, 1:2],
    color="#0277BD",
    linewidth=0.7,
    linestyle="--",
    label="Yasukawa, w/o wind",
)
ax1.plot(
    exp_data.index[startstep:].values,
    valid_hachii_nowind[:, 1:2],
    color="tomato",
    linewidth=0.7,
    linestyle="--",
    label="Hachii, w/o wind",
)
